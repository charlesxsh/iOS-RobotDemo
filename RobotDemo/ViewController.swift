...
//    }
    ...
    @IBAction func sendposition(sender: AnyObject) {
        var x:Int = Int(3000*(self.point_position.x/self.map.bounds.size.width))
        var y:Int = Int(3000*(self.point_position.y/self.map.bounds.size.width))
    ...
    func setImageToMap(grayArray:UnsafeMutablePointer<UInt8>){
        var map_image:UIImage = CVWrapper.createUIImageWithArray(grayArray)
        self.map.updateView(map_image)
    }
